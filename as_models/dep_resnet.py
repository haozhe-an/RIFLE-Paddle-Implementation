...
from .lr_schedule import fc_cosine_decay
...
class ResNet():
    def __init__(self, layers=50, is_test=True, global_name='', cyclic_num=0):
        self.layers = layers
    ...
    def net(self, input, class_dim=1000, data_format="NCHW"):
        ...
        if layers >= 50:
            ...
            if self.cyclic_num != 0:
                out = fluid.layers.fc(
                    input=pool,
                    size=class_dim,
                    bias_attr=fluid.param_attr.ParamAttr(
                        name=self.global_name + 'fc_0.b_0', learning_rate=fc_cosine_decay(0.1, 1000, 10, self.cyclic_num)),
                    param_attr=fluid.param_attr.ParamAttr(
                        name=self.global_name + 'fc_0.w_0', learning_rate = fc_consine_decay(0.1, 1000, 10, self.cyclic_num),
...
def ResNet18(is_test=True, global_name='', cyclic_num=0):
    model = ResNet(layers=18, is_test=is_test, global_name=global_name, cyclic_num=cyclic_num)
    return model
